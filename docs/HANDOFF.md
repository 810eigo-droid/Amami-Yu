...
- **スマホ用画像のカスタマイザー項目は、ユーザー環境で表示を確認できなかった（2026-09-22）。** コードは残しているが未検証扱い。クライアントには「トップ画像が鮮明になった」ことのみ伝える方針。マニュアルからはスマホ用の記述を外した。
- マニュアルは `manual/index.html` として GitHub Pages で公開（https://810eigo-droid.github.io/Amami-Yu/manual/）。印刷ボタン付き。PDF は `docs/LP編集マニュアル.pdf`。
