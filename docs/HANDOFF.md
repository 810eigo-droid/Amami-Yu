...
| bg-orange.webp（03,06,09,11）/ bg-pink.webp（05,08,13）/ bg-beige.webp（10,14） | 漆喰テクスチャ背景。白のかぶせ 22%（`--lp-bg-veil`）。bg-beige はメディアにのみ存在（リポジトリ未収録） |
...
3. クライアント確認用に **公開状態を「パスワード保護」で公開**し、確認依頼文（チャット履歴に作成済み。要点: URL・パスワード・確認ポイント5つ）を送る。
4. クライアントの修正依頼を反映。
