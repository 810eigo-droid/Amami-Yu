...
> **引き継ぎ・現状・未完了項目は `docs/HANDOFF.md` を最初に読んでください。**
...
3. **ブロックエディタの場合**: `blocks/01-fv.html` から `17-jsonld.html` まで、順番に **「カスタムHTML」ブロック** を1つ追加して中身を貼る。
   ...
   **旧エディタ（クラシックエディタ）の場合**: `blocks/ALL.html`（01〜17を結合したもの）を「コード」タブに1回で貼る。各セクションの先頭に `<!-- 01 … -->` のコメントがあるので、後から差し替えるときはその範囲を置き換える。
...
## 画像の置き場所
- `dist/lp-images.zip` を解凍し、中の画像を WordPress の「メディア > 新規追加」にアップロードすると、LPは自動でメディア側の画像を使う。同じ名前（`voice-1.webp`）や「-1」「-2」付き（`voice-1-2.webp`）が複数あるときは **一番新しいもの** を使うので、同名で上げ直すだけで差し替わる。
- メディアに無い場合は子テーマ内 `assets/lp/img` の画像を使う（フォールバック）。
- クライアントが画像を差し替えるときは、メディアに新しい画像を上げて、該当ブロックの `src` をそのURLに書き換える（マニュアル参照）。
...
- ストライプの区切り帯は雰囲気に合わないため不採用。代わりに H2／H3 に背景デザイン（ピンク→水色のグラデーション、ひし形と輪の飾り）。
- 各セクションに画像枠 `.lp-visual`（sec-03〜sec-14.webp、いまは仮画像）。生成プロンプトは `docs/image-prompts.md`。
